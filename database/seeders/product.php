...
class product extends Seeder
{
    ...
    public function run()
    {
        ...
        \App\Models\Product::forceCreate([
            'name'=>'laptop'.$i,
            'slug'=>'laptop-'.$i,
            'image'=>'storage/products/laptop/laptop-'.$i.'.jpg',
            ...
            'description'=>"A laptop or laptop computer, is a small, portable personal computer (PC) with a  form factor, typically having a thin LCD or LED computer screen mounted on the inside of the upper lid of the clamshell and an alphanumeric keyboard on the inside of the lower lid.",
             'images'=>'["storage/products/random/'.array_rand($rand).'.jpg", "storage/products/random/'.array_rand($rand).'.jpg", "storage/products/random/'.array_rand($rand).'.jpg"]'
        ])->categories()->attach(1);
        ...
        \App\Models\Product::forceCreate([
            ...
            'image'=>'storage/products/desktop/desktop-1.jpg',
             'images'=>'["storage/products/random/'.array_rand($rand).'.jpg", "storage/products/random/'.array_rand($rand).'.jpg", "storage/products/random/'.array_rand($rand).'.jpg"]'
            ])->categories()->attach(2);
        ...
        foreach(range(1,30) as $i)
            \App\Models\Product::forceCreate([
                'name'=>'mobile'.$i,
                'slug'=>'mobile-'.$i,
                'image'=>'storage/products/mobile/mobile-1.jpg',
        ...
        foreach(range(1,30) as $i)
            \App\Models\Product::forceCreate([
                'name'=>'tablet'.$i,
                'slug'=>'tablet-'.$i,
                'image'=>'storage/products/tablet/tablet-1.jpg',
                'price'=>[2099,44,3222,1244,6789,12345,2002,2000,123467][array_rand([2099,44,3222,1244,6789,12345,2002,2000,123467])]*100,
        ...
        foreach(range(1,30) as $i)
            \App\Models\Product::forceCreate([
                'name'=>'tv'.$i,
                'slug'=>'tv-'.$i,
                'image'=>'storage/products/tv/tv-1.jpg',
        ...
        foreach(range(1,30) as $i)
            \App\Models\Product::forceCreate([
                'name'=>'camera'.$i,
                'slug'=>'camera-'.$i,
                'image'=>'storage/products/camera/camera-1.jpg',
                'price'=>[2099,4444,3222,1244,6789,12345,2002,2000,123467][array_rand([2099,44,3222,1244,6789,12345,2002,2000,123467])]*100,
        ...
        foreach(range(1,30) as $i)
            \App\Models\Product::forceCreate([
                'name'=>'appliance'.$i,
                'slug'=>'appliance-'.$i,
                'image'=>'storage/products/appliance/appliance-1.jpg',
                'price'=>[2099,4444,3222,1244,6789,12345,2002,2000,123467][array_rand([2099,44,3222,1244,6789,12345,2002,2000,123467])]*100,
                'details'=>[1,2,4][array_rand([1,2,4])]."inch, ".[100,200,300][array_rand([100,200,300])]."Giga, core i5 and lense ",
